...
public class NavigationHelper {
    ...
    private static void switchScreen(JComponent currentSource, String title, JComponent newContent) {
        ...
        if (window instanceof JFrame) {
            ...
            frame.setTitle(title); // Đổi tiêu đề cửa sổ

            // 3. Vẽ lại giao diện
            frame.revalidate();
            frame.repaint();

            // Nếu bạn muốn cửa sổ tự co giãn theo nội dung mới thì mở dòng dưới ra:
            // frame.pack();
            // frame.setLocationRelativeTo(null);
        } else {
            // Trường hợp dự phòng (ví dụ chạy lần đầu chưa có frame)
    ...
    // Hàm tạo cửa sổ mới (chỉ dùng khi chưa có cửa sổ nào)
