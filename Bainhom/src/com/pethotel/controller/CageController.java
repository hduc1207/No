...
public class CageController {
    ...
    private void initEvents() {
        ...
        view.getTblCageList().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                int row = view.getTblCageList().getSelectedRow();
                if (row >= 0) {
                    view.getTxtCageID().setText(tableModel.getValueAt(row, 0).toString());
                    view.getTxtSerial().setText(tableModel.getValueAt(row, 1).toString());
                    view.getCboType().setSelectedItem(tableModel.getValueAt(row, 2).toString());
                    view.getTxtPrice().setText(tableModel.getValueAt(row, 3).toString());
                    view.getCboStatus().setSelectedItem(tableModel.getValueAt(row, 4).toString());
                }
            }
        });
    ...
    private CageDTO getCageFromForm() {
        try {
            ...
            c.setCageName(view.getTxtSerial().getText()); // Mapping txtSerial -> CageName
